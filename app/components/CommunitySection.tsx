...
export function CommunitySection() {
  ...
  const categories = [
    ...
    { key: "tech", label: "Cyber & AI Tech", icon: Terminal },
    { key: "islamic", label: "Kader & Keislaman", icon: BookOpen },
    { key: "creative", label: "Media & Studio", icon: Video },
    { key: "sports", label: "Esports & Games", icon: Gamepad2 },
    { key: "leadership", label: "Kepemimpinan", icon: Compass },
  ];
    ...
    <section id="komunitas" className="py-20 relative bg-cyber-grid">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          {/* Type Switcher */}
          <div className="inline-flex items-center gap-1 p-1 rounded-2xl bg-slate-100/90 dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800 self-start md:self-auto text-xs font-semibold overflow-x-auto scrollbar-none max-w-full">
            {[
              { key: "all" as const, label: "Semua", icon: Sparkles, count: communityData.length },
              {
                key: "ormawa" as const,
                label: "Ormawa",
                icon: ShieldCheck,
                count: communityData.filter((c) => c.type === "ormawa").length,
              },
              {
                key: "ukm" as const,
                label: "UKM Virtual",
                icon: Laptop,
                count: communityData.filter((c) => c.type === "ukm").length,
              },
            ].map((t) => {
              const Icon = t.icon;
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl transition-all duration-200 whitespace-nowrap flex items-center gap-2 shrink-0 select-none active:scale-95 ${
                    isActive
        ...
        <div className="mb-12 rounded-3xl overflow-hidden glass-card border border-teal-500/30 shadow-2xl grid grid-cols-1 lg:grid-cols-12">
          <div className="lg:col-span-7 relative h-64 sm:h-80 lg:h-auto overflow-hidden">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t lg:bg-gradient-to-r from-slate-950/85 via-slate-950/35 to-transparent pointer-events-none" />
            <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 py-1.5 px-2.5 sm:py-2 sm:px-3.5 rounded-xl bg-slate-950/80 backdrop-blur-md border border-white/20 text-[11px] sm:text-xs font-semibold text-white flex items-center gap-2 shadow-lg max-w-[calc(100%-1.5rem)] sm:max-w-none">
              <span className="relative flex h-2 w-2 shrink-0" aria-hidden="true">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500" />
              </span>
              <span className="truncate">
                Live War Room & Hackathon <span className="hidden xs:inline">Coding Session</span>
              </span>
            </div>
          </div>
        ...
        {/* Category Filter Badges inside unified Segmented Control Bar */}
        <div className="flex items-center gap-1.5 p-1 rounded-2xl bg-slate-100/90 dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800 text-xs font-semibold overflow-x-auto scrollbar-none mb-8 max-w-full">
          {categories.map((cat) => {
            const Icon = cat.icon;
            const count =
              cat.key === "all"
                ? selectedType === "all"
                  ...
                  : communityData.filter((c) => c.type === selectedType).length
